...
uint get_total_signal_no() {return 229;}

uint get_number_of_components() {return 65;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 85;}
...
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[32];
}
...
for (uint i = 0; i < 16; i++) {
std::string new_cmp_name = "positionAns"+ctx->generate_position_array(aux_dimensions, 2, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
...
uint aux_dimensions[2] = {4,4};
for (uint i = 0; i < 16; i++) {
std::string new_cmp_name = "matchAns"+ctx->generate_position_array(aux_dimensions, 2, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
IsEqual_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 6 ;
aux_cmp_num += 2;
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 24
...
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = (((4 * Fr_toInt(&lvar[0])) + (1 * Fr_toInt(&lvar[2]))) + 0);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((4 * Fr_toInt(&lvar[0])) + (1 * Fr_toInt(&lvar[2]))) + 0)]);
}
...
{
uint cmp_index_ref = (((4 * Fr_toInt(&lvar[0])) + (1 * Fr_toInt(&lvar[2]))) + 16);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((4 * Fr_toInt(&lvar[0])) + (1 * Fr_toInt(&lvar[2]))) + 16)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
IsEqual_1_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[(((4 * Fr_toInt(&lvar[0])) + (1 * Fr_toInt(&lvar[2]))) + 0)]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[(((4 * Fr_toInt(&lvar[0])) + (1 * Fr_toInt(&lvar[2]))) + 16)]].signalStart + 0]); // line circom 34
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 34. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
PFrElement aux_dest = &lvar[1];
// load src
...
Fr_eq(&expaux[0],&circuitConstants[2],&circuitConstants[2]); // line circom 37
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 37. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
PFrElement aux_dest = &lvar[0];
// load src
